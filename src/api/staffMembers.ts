...
export const staffMembersApi = {
	...
	addStaffMember: async (data: CreateStaffMemberReq): Promise<BaseAPIResponse & { data: { member: StaffMember } }> => {
		const response = await apiClient.post(`${API_ROUTES.STAFF_MEMBERS}/new`, data);
		return response.data;
	},

	updateStaffMember: async (id: string, data: UpdateStaffMemberReq): Promise<BaseAPIResponse & { data: { member: StaffMember } }> => {
