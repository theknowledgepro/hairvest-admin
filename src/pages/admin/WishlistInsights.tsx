...
import { Heart, User, Package, Calendar, Search, Loader2 } from 'lucide-react';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/ui/table';
import { Button } from '@/components/ui/button';
import { useWishlistInsightsQuery } from '../../hooks/useInsights';
...
export const WishlistInsights: React.FC = () => {
	...
	const { data: insightsResponse, isLoading } = useWishlistInsightsQuery(page, limit, debouncedSearch);
	...
	return (
		<div className='animate-in fade-in slide-in-from-bottom-4 duration-500'>
			<div className='flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8'>
				<div>
					<h2 className='text-3xl font-bold tracking-tight text-white'>Wishlist Insights</h2>
					<p className='text-neutral-400 mt-1'>Understand customer preferences by tracking items they've saved to their wishlist.</p>
				</div>
			</div>
				...
				<CardHeader className='border-b border-neutral-800 p-4'>
					<div className='flex flex-col md:flex-row md:items-center justify-between gap-4'>
						<CardTitle className='text-lg font-semibold text-white flex items-center gap-2'>
							...
							<span className='ml-2 px-2 py-0.5 text-[12px] leading-tight font-medium rounded-[6px] bg-red-500/10 text-red-400 border border-red-500/20'>
								{totalResults} Total
							</span>
						</CardTitle>
						<div className='relative w-full md:w-72'>
							...
							<Input
								...
								className='bg-neutral-950 border-neutral-800 pl-10 focus-visible:ring-red-500'
								value={searchInput}
								onChange={(e) => {
				...
				<CardContent className='p-0'>
					{isLoading ?
						<div className='p-24 flex flex-col items-center justify-center text-neutral-500'>
							...
							<p className='animate-pulse'>Fetching wishlist data...</p>
						</div>
					: insights.length === 0 ?
						<div className='p-20 flex flex-col items-center justify-center text-neutral-500'>
							...
							<p className='text-sm mt-1 text-center max-w-sm'>No customers have added your products to their wishlist yet.</p>
						</div>
					:	<>
							<Table>
								<TableHeader className='bg-neutral-800/30'>
									<TableRow className='hover:bg-transparent border-neutral-800'>
										...
										<TableHead className='text-neutral-400'>Price Point</TableHead>
										<TableHead className='text-neutral-400 text-right'>Saved On</TableHead>
									</TableRow>
								</TableHeader>
								<TableBody>
									{insights.map((item, index) => (
										<TableRow key={item.key} className='border-neutral-800 hover:bg-neutral-800/20 transition-colors group'>
											...
											<TableCell>
												<span className='font-semibold text-white'>{formatCurrency(item.product?.amount || 0)}</span>
											</TableCell>
